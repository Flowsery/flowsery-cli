...
beforeEach(() => {
  ...
  home = mkdtempSync(join(tmpdir(), 'fsy-creds-'));
...
describe('token formatting', () => {
  it('keeps the prefix plus four characters', () => {
    expect(tokenPrefixOf('flow_9f3a1b2c3d')).toBe('flow_9f3a');
    expect(tokenPrefixOf('flow_ws_9f3a1b2c')).toBe('flow_ws_9f3a');
    expect(redactToken('flow_ws_ab12cd34')).toBe('flow_ws_ab12…');
...
describe('credentials file', () => {
  ...
  it('writes 0600 through a temp file and leaves nothing behind', () => {
    saveProfile('default', { token: 'flow_ws_9f3a1b2c', apiUrl: 'https://example.test/api/v1', label: 'personal' });
    ...
    expect(stored.profiles.default.tokenPrefix).toBe('flow_ws_9f3a');
  ...
  it('refuses a group- or world-readable file and says how to fix it', () => {
    saveProfile('default', { token: 'flow_ws_9f3a1b2c' });
  ...
  it('keeps several profiles and tracks the current one', () => {
    saveProfile('default', { token: 'flow_ws_1111aaaa' });
    saveProfile('acme', { token: 'flow_ws_2222bbbb', label: 'Acme workspace' });
  ...
  it('does not make a saved profile current when asked not to', () => {
    saveProfile('default', { token: 'flow_ws_1111aaaa' });
    saveProfile('ci', { token: 'flow_ws_3333cccc' }, { makeCurrent: false });
    expect(readCredentials().current).toBe('default');
  });
  ...
  it('reads a file written before the type discriminator existed', () => {
    ...
    writeFileSync(
      credentialsPath(),
      JSON.stringify({ version: 1, current: 'default', profiles: { default: { token: 'flow_ws_4444dddd' } } }),
      { mode: 0o600 },
    );
    const profile = readCredentials().profiles.default;
    expect(profile.type).toBe('token');
    expect(profile.tokenPrefix).toBe('flow_ws_4444');
  });
    ...
    writeCredentials({
      ...
      profiles: {
        default: {
          ...
          authkitDomain: 'auth.flowsery.com',
        },
      },
    });
    const profile = readCredentials().profiles.default;
