...
REPO="Flowsery/flowsery-cli"
BIN="flowsery"
INSTALL_DIR="${FLOWSERY_INSTALL_DIR:-$HOME/.local/bin}"
  ...
  case "$os" in
    ...
    Linux)
      ...
      case "$arch" in
        ...
        arm64 | aarch64)
          if [ "$libc" = musl ]; then
            die "no musl arm64 binary is published. Install with: npm i -g @flowsery/cli"
          fi
          printf 'linux_arm64\n'
          ;;
      ...
      ;;
    *)
      die "unsupported operating system: $os. On Windows use: npm i -g @flowsery/cli"
      ;;
  esac
}
...
[ -w "$INSTALL_DIR" ] || die "$INSTALL_DIR is not writable. Set FLOWSERY_INSTALL_DIR to a directory you own, or rerun with sudo to install into /usr/local/bin"
