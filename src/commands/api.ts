  ...
  for (const pair of pairs) {
    ...
    if (index <= 0) {
      throw usageError(
        ...
        `${PRODUCT.binName} api /issues --query status=open --query limit=5`,
      );
    }
...
export function registerApiCommand(program: Command): void {
  program
    .command('api')
    .argument('<path>', 'API path such as /overview, or an HTTP method when a path follows')
      ...
      async (
        ...
        if (/^[a-z][a-z0-9+.-]*:\/\//i.test(path)) {
          throw usageError(
            ...
            `This CLI only ever talks to the resolved API host. Try: ${PRODUCT.binName} api /overview`,
          );
        }
