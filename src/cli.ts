...
if (isBelow(process.versions.node, NODE_FLOOR)) {
  process.stderr.write(
    `flowsery needs Node ${NODE_FLOOR} or newer, this is Node ${process.versions.node}. ` +
...
const MODULES: Array<{ names: string[]; load: () => Promise<Registrar> }> = [
  ...
  {
    names: ['site', 'sites'],
    load: async () => (await import('./commands/site.js')).registerSiteCommands,
  },
  {
    names: ['stats', 'breakdown'],
    load: async () => (await import('./commands/stats.js')).registerStatsCommands,
  },
  {
    names: ['goal', 'goals'],
    load: async () => (await import('./commands/goal.js')).registerGoalCommands,
  },
  {
    names: ['payment', 'payments'],
    load: async () => (await import('./commands/payment.js')).registerPaymentCommands,
  },
  {
    names: ['issue', 'issues'],
    load: async () => (await import('./commands/issue.js')).registerIssueCommands,
  },
  {
    names: ['visitor', 'visitors', 'live'],
    load: async () => (await import('./commands/realtime.js')).registerRealtimeCommands,
  },
];
